...
test_that("`ts.split` function", {
  ...
  expect_equal(length(split.list$out.of.sample), length(AirPassengers) * 
    frac)
  expect_equal(length(split.list$in.sample), length(AirPassengers) * 
    (1 - frac))
  ...
  expect_equal(length(na.omit(split.ts[, "out.of.sample"])), 
    length(AirPassengers) * frac)
  expect_equal(length(na.omit(split.ts[, "in.sample"])), length(AirPassengers) * 
    (1 - frac))
